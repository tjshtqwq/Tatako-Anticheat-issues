---
name: Tatako Generic Issue
about: Tatako Generic issues
---

# Tatako Issue
### PLEASE SELECT ISSUE LABEL!
### Bypasses Or False Flags: Need TatakoLogs and Video
### Error or Bugs: Need ServerLogs, TatakoLogs, and Video
### Both need：Tatako Version, Minecraft Server Version, MC Server Core (e.g. PaperSpigot)
### Please Describe your issue.
### Supported Chinese or English. But we suggest you using chinese to avoid ambiguity.
# Tatako 问题追踪器
### 务必选择问题标签！
### 绕过或误判：需要Tatako日志和视频
### 错误或Bug：需要服务器日志，Tatako日志和视频
### 都需要：Tatako版本号，MC服务器版本，MC服务器核心名称（例如PaperSpigot）
### 请详细描述你的问题。
### 支持中文或英文。但我们建议你使用中文避免歧义。

`Change - [ ] to - [X] to check the checkboxes below.`
`将 - [ ] 改成 - [X] 让复选框勾选`

# Please Check 请检查：
- [ ] Tatako is up-to-date Tatako是最新的
- [ ] Tatako is running on a 1.8 ~ 1.21.4 spigot server. Tatako运行于1.8 ~ 1.21.4的Spigot服务器上
- [ ] No plugins are incompatible with Tatako. 没有不兼容的插件
- [ ] The issue happens with default config.yml 默认配置仍复现

# Generic 通用项
### You must fill the following items no matter what you report. 无论你报告什么，以下项目必填
**Server Version 服务器版本**


**Tatako Version Tatako版本**


**Client Version 客户端版本**


**Plugin List 插件列表**



# Bypass 绕过


**Hacked Client 作弊客户端名称**


**Is this Hacked Client private? 作弊客户端是否私有**


**What check bypassed 绕过了什么**


**Video for bypass 绕过视频**


**Check alerts and information (Using Tatako.log) Tatako警报和信息（使用Tatako.log）**



# False Flag 误判
- [ ] Using closed-source client (lunar,badlion,etc.) or optifine fastmath? 使用闭源客户端（例如Lunar Badlion等），或开启了Optifine的FastMath？
- [ ] Special gameplay plugin (MCMMO, etc.)? 特殊的玩法插件（例如 MCMMO）

**Client used 客户端名称**


**Check alerts and information (Using Tatako.log) Tatako警报和信息（使用Tatako.log）**


**Video for false flag 误报视频**



# Bug
- [ ] Using closed-source server? 使用闭源服务端？
- [ ] Using special/private server? 使用特殊/私有服务端？
**Full server log and tatako log**


